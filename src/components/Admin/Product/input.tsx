...
const NumericInput: React.FC<NumericInputProps> = (props) => {
    ...
    const handleBlur = () => {
        ...
        if (max && parseFloat(valueTemp) > max) { // Kiểm tra max tồn tại trước khi sử dụng nó
            onChange(max.toString());
        } else if (min && parseFloat(valueTemp) < min) { // Kiểm tra min tồn tại trước khi sử dụng nó
