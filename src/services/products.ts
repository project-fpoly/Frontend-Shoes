...
export const addProduct = async (product: IProduct): Promise<IProduct | null> => {
  try {
    const response: AxiosResponse<IProduct> = await instance.post("/api/product", product);
    notification.success({ message: "Product added successfully" });
    return response.data || response;
  } catch (error) {
    console.log(error);
    const customError = error as CustomError;
    const errorMessage = customError.response?.data?.message || "Error while adding product.";
    notification.error({ message: errorMessage });
    throw new Error("Error while adding product.");
  }
};

export const updatePrroduct = async (id: string, product: IProduct): Promise<IProduct | null> => {
  try {
    const response: AxiosResponse<IProduct> = await instance.put(`/api/product/${id}`, product);
    notification.success({ message: "Product updated successfully" });
    return response.data || response;
  } catch (error) {
    console.log(error);
    const customError = error as CustomError;
    const errorMessage = customError.response?.data?.message || "Error while updating product.";
    notification.error({ message: errorMessage });
...
export const deleteProduct = async (id: string): Promise<IProduct | null> => {
  try {
    const response: AxiosResponse<IProduct> = await instance.delete(`/api/product/${id}`);
    notification.success({ message: "Product deleted successfully." });
    return response.data || response;
  } catch (error) {
    console.log(error);
    const customError = error as CustomError;
    const errorMessage = customError.response?.data?.message || "Error while deleting product.";
    notification.error({ message: errorMessage });
