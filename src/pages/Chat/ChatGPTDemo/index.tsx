...
const ChatGPTDemo: React.FC<ChatGPTDemoProps> = ({ onClose }) => {
  ...
  useEffect(() => {
    ...
    // Lấy dữ liệu từ API
    axios
      .get('https://660edb22356b87a55c504eef.mockapi.io/chatgpt')
      .then((response) => {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (!initialMessagesSent) {
        ...
        messagesToSend = [...messages, { role: 'user', content: userInput }]
      }
      ...
      try {
        ...
        message.error(
          'Tính năng đã được bảo mật, hãy liên hệ với admin để mở khóa tính năng!!',
        )
  ...
  const steps: TourProps['steps'] = [
    {
      title: 'Gửi tin nhắn',
      description:
        'Đây là tính năng bảo mật, bạn cần có xác nhận của admin để dùng tính năng này. Tính năng giúp tin nhắn của bạn đến AI, bạn có thể tra cứu thông tin về giày',
  ...
  return (
    <>
      ...
      <ul>
        {messages.map((message, index) => (
          <li key={index} className="py-3 sm:py-4">
            <div
              className={`flex items-center space-x-4 ${messageClasses(message.role)}`}
            >
              ...
              >
                <span style={{ color: 'white' }}>
                  {message.role === 'user' ? roleUser : 'Assistant:'}
                </span>{' '}
                {message.content}
              </p>
            </div>
      ...
      <div className="mt-6">
        ...
        <Button className="ml-3" onClick={onClose} ref={tourRef2}>
          Close
        </Button>
