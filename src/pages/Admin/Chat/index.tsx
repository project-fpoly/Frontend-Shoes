...
const SettingsPage = () => {
    ...
    const handleSubmit = async (values:any) => {
        setLoading(true);
        try {
            await axios.put('https://660edb22356b87a55c504eef.mockapi.io/chatgpt/1', values);
    ...
    return (
        <div>
            <button onClick={() => setModalVisible(true)}>Mở khóa tính năng ChatGPT</button> {/* Nút để mở modal */}
            <ChatsPage />
            <Modal
                title="Mở khóa"
                open={modalVisible}
            ...
            >
                {loading ? (
                <Spin />
            ) :( <UpdateSettingsForm initialValues={initialValues} onSubmit={handleSubmit} />)}
