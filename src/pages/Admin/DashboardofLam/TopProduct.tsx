...
const TopProduct = ({ id }: { id: string }) => {
  ...
  useEffect(() => {
    getChart(id, '2003-03-26', '2024-03-26', 'day').then((resp) => {
      ...
      const sortedData = top5product.sort((a: any, b: any) => b.sold_count - a.sold_count);
      setData(sortedData);
    });
  }, [id]);
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'image',
      render: (image: string[]) => (
        <img src={image[0]} alt="Product" style={{ maxWidth: '70px', maxHeight: '70px' }} />
      ),
      width: 100,
    },
  ];
  ...
  return (
    <>
      ...
      <Table style={{fontSize:"10px"}} dataSource={data} size='small' columns={columns} pagination={false} />
    </>
  );
};
